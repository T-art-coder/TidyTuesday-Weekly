...
p <- rent[!is.na(rent$price), ] %>%
  ...
  scale_x_continuous(breaks = c(2000, 2005, 2010, 2015, 2018))+
  my_pretty_theme+
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        panel.grid.major = element_blank(),
        legend.position="none")+
  labs(title = "Rental Housing in California: An Overview", subtitle = "Average rent prices based on Craiglist data, 2000-2018", caption = "TidyTuesday W-27 2022 | K. Pennington Rental Housing Data | by @TimirZolkin")
...
```{r}
rent[!is.na(rent$sqft), ] %>% select(sqft) %>% max()/1000
```
...
```{r}
p_beds_e <- rent[!is.na(rent$price), ] %>%
  #filter(year == 2018) %>%
  drop_na(county) %>%
  ggplot(aes(x = beds)) +
...
p_baths_e <- rent[!is.na(rent$price), ] %>%
  #filter(year == 2018) %>%
  drop_na(county) %>%
  ggplot(aes(x = baths)) +
  geom_histogram(fill = "navyblue", binwidth = 1)+
...
p_rents_e <- rent[!is.na(rent$sqft), ] %>%
  #filter(year == 2018) %>%
  drop_na(county) %>%
  ggplot(aes(x = sqft)) +
  geom_histogram(fill = "navyblue", binwidth = 200)+
  theme_pubclean()

ggarrange(p_beds_e, p_baths_e, p_rents_e)
```


```{r fig.height=6, fig.width=10}
sqft_p_1 <- rent[!is.na(rent$sqft), ] %>%
  filter(county != "NA") %>%
  filter(sqft < quantile(sqft, 0.99)) %>%
  ggplot(aes (x = factor(county), y = sqft, color = county)) +
  geom_boxplot() +
  theme_pubclean(base_family = "Roboto Condensed", base_size = 12) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        plot.title = element_text(face = "bold", size = rel(1.7)),
  ...
  labs(title = "Appartment Area in the San Francisco neighborhood, sq.feet",
       subtititle = "Data in sq. feet from Dr.Pennington")

sqft_p_2 <- rent[!is.na(rent$sqft), ] %>%
  filter(county != "NA") %>%
  filter(sqft < quantile(sqft, 0.99)) %>%
  ggplot(aes (x = factor(county), y = sqft, color = county)) +
  geom_violin( size = 1, alpha = .5) +
  geom_jitter(alpha = .02, width = .3, colour = "grey90") +
  theme_pubclean(base_family = "Roboto Condensed", base_size = 12) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        plot.title = element_text(face = "bold", size = rel(1.7)),
        axis.text.x = element_text(face = "bold", size = rel(1.25)),
        axis.text.y = element_text(face = "bold", size = rel(1.2)),
        plot.caption = element_text(face = "bold", colour ="grey 79", size = rel(1))
        )+
  scale_y_continuous() +
  scale_color_brewer(palette = "Paired", guide = "none")+
  labs(caption = "TidyTuesday W-27 2022 | K. Pennington Rental Housing Data | by @TimirZolkin")
  

ggarrange(sqft_p_1,sqft_p_2, nrow = 2, ncol = 1 )
```
